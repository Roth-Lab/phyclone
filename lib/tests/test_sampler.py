...
class Test(unittest.TestCase):
    def testName(self):
        ...
        log_likelihood_func = lambda x, p: sum([log_binomial_pdf(x_sample[0], x_sample[1], p_sample) for x_sample, p_sample in zip(x, p)])

        node_agg_func = sum
        
        sampler = Sampler(alpha, data, num_particles, log_likelihood_func, param_proposal_func, node_agg_func, param_prior, num_implicit_particles=int(1e4), randomise_data=False)

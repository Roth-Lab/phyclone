...
def get_exact_posterior(data, tree_dist, factorial_arr, alpha=1.0):
    ...
    for num_nodes in forests:
        for parent_idxs in get_oriented_forests(num_nodes):
            for clusters in forests[num_nodes]:
                t = get_fscrp_tree(alpha, grid_size, clusters, parent_idxs, factorial_arr)
...
def get_fscrp_tree(alpha, grid_size, clusters, parent_pointers, factorial_arr):
    ...
    tree = Tree(grid_size, factorial_arr)
